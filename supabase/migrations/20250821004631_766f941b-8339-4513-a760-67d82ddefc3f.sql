-- Verificar e criar constraint única para evitar duplicatas (syntax corrigida)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'uniq_freq' 
        AND table_name = 'frequencias'
    ) THEN
        ALTER TABLE frequencias ADD CONSTRAINT uniq_freq UNIQUE (aluno_id, aula_id);
    END IF;
END $$;
