-- Corrigir função get_redacoes_corretor_detalhadas para usar colunas corretas
  ...
  SELECT 
    ...
    CASE 
      WHEN r.corretor_id_1 = ci.id THEN 
        CASE 
          WHEN r.status_corretor_1 IS NULL THEN 'pendente'
          ELSE r.status_corretor_1
        END
      WHEN r.corretor_id_2 = ci.id THEN 
        CASE 
          WHEN r.status_corretor_2 IS NULL THEN 'pendente'
  ...
  SELECT 
    ...
    CASE 
      WHEN rs.corretor_id_1 = ci.id THEN 
        CASE 
          WHEN rs.status_corretor_1 IS NULL THEN 'pendente'
          ELSE rs.status_corretor_1
        END
      WHEN rs.corretor_id_2 = ci.id THEN 
        CASE 
          WHEN rs.status_corretor_2 IS NULL THEN 'pendente'
  ...
  SELECT 
    ...
    CASE 
      WHEN re.corretor_id_1 = ci.id THEN 
        CASE 
          WHEN re.status_corretor_1 IS NULL THEN 'pendente'
          ELSE re.status_corretor_1
        END
      WHEN re.corretor_id_2 = ci.id THEN 
        CASE 
          WHEN re.status_corretor_2 IS NULL THEN 'pendente'
