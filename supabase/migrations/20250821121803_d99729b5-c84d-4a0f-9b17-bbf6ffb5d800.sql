-- Corrigir funções de presença com aliases únicos para evitar ambiguidade
CREATE OR REPLACE FUNCTION public.registrar_entrada(p_email text, p_aula_id uuid)
RETURNS TABLE(r_aluno_id uuid, r_aula_id uuid, r_entrada_at timestamp with time zone, r_saida_at timestamp with time zone)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
AS $function$
...
BEGIN
  ...
  -- Retornar os valores com aliases únicos
  RETURN QUERY VALUES (
...
-- Corrigir função registrar_saida com aliases únicos
