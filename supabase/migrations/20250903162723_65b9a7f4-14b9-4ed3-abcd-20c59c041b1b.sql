-- Corrigir função para mostrar apenas redações pendentes para o corretor específico
...
AS $function$
  ...
  -- Redações enviadas regulares - apenas as que o corretor deve corrigir
  SELECT 
    r.id,
  ...
  WHERE (
    (r.corretor_id_1 = ci.id AND COALESCE(r.status_corretor_1, 'pendente') != 'corrigida') OR
    (r.corretor_id_2 = ci.id AND COALESCE(r.status_corretor_2, 'pendente') != 'corrigida')
  )
  
  UNION ALL
  
  -- Redações de simulado - apenas as que o corretor deve corrigir
  SELECT 
    rs.id,
  ...
  CROSS JOIN corretor_info ci
  WHERE (
    (rs.corretor_id_1 = ci.id AND COALESCE(rs.status_corretor_1, 'pendente') != 'corrigida') OR
    (rs.corretor_id_2 = ci.id AND COALESCE(rs.status_corretor_2, 'pendente') != 'corrigida')
  )
  
  UNION ALL
  
  -- Redações de exercício - apenas as que o corretor deve corrigir
  SELECT 
    re.id,
  ...
  CROSS JOIN corretor_info ci
  WHERE (
    (re.corretor_id_1 = ci.id AND COALESCE(re.status_corretor_1, 'pendente') != 'corrigida') OR
    (re.corretor_id_2 = ci.id AND COALESCE(re.status_corretor_2, 'pendente') != 'corrigida')
  )
