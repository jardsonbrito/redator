-- Atualizar a função get_redacoes_corretor_detalhadas para incluir campos de renderização
CREATE OR REPLACE FUNCTION public.get_redacoes_corretor_detalhadas(corretor_email text)
 RETURNS TABLE(id uuid, tipo_redacao text, nome_aluno text, email_aluno text, frase_tematica text, data_envio timestamp with time zone, texto text, status_minha_correcao text, eh_corretor_1 boolean, eh_corretor_2 boolean, redacao_manuscrita_url text, turma text, render_status text, render_image_url text, thumb_url text)
 LANGUAGE sql
...
AS $function$
  ...
  SELECT 
    ...
    r.turma,
    COALESCE(r.render_status, 'pending') as render_status,
    r.render_image_url,
    r.thumb_url
