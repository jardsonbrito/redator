-- Adicionar suporte para soft delete em todas as tabelas de redações
-- Isso permite recuperar textos de redações que foram canceladas

-- Adicionar coluna deleted_at à tabela redacoes_enviadas
ALTER TABLE public.redacoes_enviadas
ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP WITH TIME ZONE DEFAULT NULL;

-- Adicionar coluna deleted_at à tabela redacoes_exercicio
ALTER TABLE public.redacoes_exercicio
ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP WITH TIME ZONE DEFAULT NULL;

-- Adicionar coluna deleted_at à tabela redacoes_simulado
ALTER TABLE public.redacoes_simulado
ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP WITH TIME ZONE DEFAULT NULL;

-- Criar índices para melhorar performance das queries que filtram por deleted_at
CREATE INDEX IF NOT EXISTS idx_redacoes_enviadas_deleted_at
ON public.redacoes_enviadas(deleted_at)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_redacoes_exercicio_deleted_at
ON public.redacoes_exercicio(deleted_at)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_redacoes_simulado_deleted_at
ON public.redacoes_simulado(deleted_at)
WHERE deleted_at IS NULL;

-- Atualizar a função get_student_redacoes_com_status_finalizado para filtrar soft deletes
DROP FUNCTION IF EXISTS public.get_student_redacoes_com_status_finalizado(text);
...
RETURNS TABLE(
  ...
  exercicio_id uuid,
  turma text,
  processo_seletivo_candidato_id uuid
)
LANGUAGE sql
...
AS $function$
  ...
  -- Redações regulares e de processo seletivo da tabela redacoes_enviadas
  SELECT
    r.id,
    r.frase_tematica,
    ...
    r.data_envio,
    -- Manter o status original (incluindo 'reenvio')
    r.status as status,
    r.corrigida,
    ...
    r.turma,
    r.processo_seletivo_candidato_id
  ...
  AND r.tipo_envio IN ('regular', 'visitante', 'processo_seletivo')
  ...
  SELECT
    ...
    NULL::uuid as processo_seletivo_candidato_id
...
COMMENT ON COLUMN public.redacoes_enviadas.deleted_at IS 'Timestamp de quando a redação foi cancelada/deletada (soft delete). NULL significa que a redação está ativa.';
COMMENT ON COLUMN public.redacoes_exercicio.deleted_at IS 'Timestamp de quando a redação foi cancelada/deletada (soft delete). NULL significa que a redação está ativa.';
COMMENT ON COLUMN public.redacoes_simulado.deleted_at IS 'Timestamp de quando a redação foi cancelada/deletada (soft delete). NULL significa que a redação está ativa.';
