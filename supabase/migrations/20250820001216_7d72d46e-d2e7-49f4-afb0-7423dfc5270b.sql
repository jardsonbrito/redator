...
DROP POLICY IF EXISTS "Public access to essay renders" ON storage.objects;
CREATE POLICY "Public access to essay renders" 
ON storage.objects 
FOR SELECT 
USING (bucket_id = 'essay-renders');

-- Política para corretor/admin fazer upload das imagens renderizadas
DROP POLICY IF EXISTS "Corretor can upload essay renders" ON storage.objects;
CREATE POLICY "Corretor can upload essay renders" 
ON storage.objects 
FOR INSERT 
WITH CHECK (
