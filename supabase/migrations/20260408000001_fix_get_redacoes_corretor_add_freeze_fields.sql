-- Adicionar campos de congelamento à RPC get_redacoes_corretor_detalhadas
-- Motivo: corrigida, congelada e data_descongelamento não eram retornados,
-- causando alerta "Redação Congelada" mesmo em redações já corrigidas.
...
AS $function$
  ...
  -- Redações de exercício (inclui Produção Guiada e Frase Temática)
  SELECT
    re.id,
...
COMMENT ON FUNCTION public.get_redacoes_corretor_detalhadas(text) IS
  'Retorna todas as redações atribuídas a um corretor (exceto deletadas). '
  'Inclui corrigida, congelada e data_descongelamento para controle de freeze no frontend. '
  'Inclui exercicio_tipo para bifurcar o formulário de correção (ex: Produção Guiada).';
