-- Migration: Corrigir nome do aluno para mostrar nome real ao invés de "aluno"
-- Data: 2025-10-04
-- Descrição: Atualiza get_redacoes_corretor_detalhadas para buscar nome do perfil do aluno
...
AS $function$
  ...
  SELECT
    ...
    COALESCE(p.nome, r.nome_aluno) as nome_aluno,
    r.email_aluno,
    r.frase_tematica,
    r.data_envio,
  ...
  LEFT JOIN public.profiles p ON p.email = r.email_aluno
  ...
  SELECT
    ...
    COALESCE(p.nome, rs.nome_aluno) as nome_aluno,
    rs.email_aluno,
    s.frase_tematica,
    rs.data_envio,
    rs.texto,
  ...
  CROSS JOIN corretor_info ci
  LEFT JOIN public.profiles p ON p.email = rs.email_aluno
  ...
  SELECT
    ...
    COALESCE(p.nome, re.nome_aluno) as nome_aluno,
    re.email_aluno,
  ...
  CROSS JOIN corretor_info ci
  LEFT JOIN public.profiles p ON p.email = re.email_aluno
...
COMMENT ON FUNCTION public.get_redacoes_corretor_detalhadas(text) IS
  'Retorna todas as redações atribuídas a um corretor com nome real do aluno do perfil, incluindo URLs de imagens manuscritas e geradas';
