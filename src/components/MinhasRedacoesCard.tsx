...
import { FileText, Calendar, User, Lock, Eye } from "lucide-react";
...
export const MinhasRedacoesCard = () => {
  const { studentProfile, isAuthenticated } = useSupabaseAuth();
  const { toast } = useToast();

  // Estados para visualização segura de redações
  ...
  // Query segura - apenas dados básicos sem informações sensíveis
  const { data: minhasRedacoes, isLoading } = useQuery({
    queryKey: ['student-redacoes', studentProfile?.email],
    queryFn: async () => {
      if (!studentProfile?.email) return [];

      console.log('🔍 Buscando redações do aluno:', studentProfile.email);
  ...
  const handleViewRedacao = (redacao: StudentRedacao) => {
    console.log('🔐 Iniciando fluxo seguro de visualização');
  ...
  const handleEmailAuth = async () => {
    ...
    try {
      ...
      const redacaoBasica = minhasRedacoes?.find(r => r.id === selectedRedacaoId);
      if (!redacaoBasica) {
      ...
      <div className="mb-8">
        <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-xl overflow-hidden">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground font-medium">
                      Suas redações enviadas com total segurança - {studentProfile?.nome}
                    </p>
                  </div>
                </div>
                <Link to="/student/redacoes">
                  <Button 
                    variant="outline" 
                    size="sm"
          ...
          <CardContent className="space-y-4">
            ...
            ) : !minhasRedacoes || minhasRedacoes.length === 0 ? (
              <div className="text-center py-8">
                <FileText className="w-12 h-12 text-muted-foreground mx-auto mb-3" />
                <p className="text-muted-foreground mb-2">Você ainda não enviou nenhuma redação</p>
                <p className="text-sm text-muted-foreground">
                  Suas redações aparecerão aqui quando você enviar alguma
                </p>
              </div>
            ) : (
              <>
                {minhasRedacoes.slice(0, 3).map((redacao) => (
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap gap-2 mb-3">
                            <Badge className={redacao.corrigida ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}>
                              {redacao.corrigida ? "🔐 Corrigido" : "⏳ Aguardando"}
                            </Badge>
