  ...
  const handleSubmit = async (action: ActionType) => {
    ...
    try {
      ...
      const { error } = editData
        ...
        : await supabase.from('lousa').insert([{ ...lousaData, created_by: corretor.id }]);
      ...
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ['corretor-lousas'] }),
      ...
      if (error instanceof Error) {
        if (error.message?.includes('row-level security')) {
          errorMessage = 'Erro de permissão: Verifique se você está logado como corretor.';
